    ...
    <div className="relative mx-auto flex h-auto min-h-screen w-full max-w-2xl flex-col overflow-x-hidden">
      <header className="sticky top-0 z-10 flex h-16 shrink-0 items-center justify-between border-b border-white/10 bg-background-light/80 px-4 backdrop-blur-sm dark:bg-background-dark/80">
        <div className="flex size-10 shrink-0 items-center justify-start text-zinc-900 dark:text-white">
          <button onClick={onClose}><Icon name="close" className="text-2xl" /></button>
        </div>
        <h1 className="flex-1 text-center text-lg font-bold leading-tight tracking-[-0.015em] text-zinc-900 dark:text-white">Add Server Configuration</h1>
        <div className="flex w-10 shrink-0 items-center justify-end"></div>
      </header>
      <main className="flex-1 pb-40">
        <div className="p-4">
          <section>
            <h2 className="px-4 pb-3 pt-5 text-[22px] font-bold leading-tight tracking-[-0.015em] text-zinc-900 dark:text-white">Server Configuration</h2>
            <div className="flex flex-col gap-4 rounded-xl bg-white p-4 dark:bg-zinc-900/50">
              <label className="flex flex-col">
                <p className="pb-2 text-base font-medium leading-normal text-zinc-700 dark:text-zinc-300">Configuration (JSON)</p>
                <textarea
              ...
              <p className="px-1 text-sm text-zinc-600 dark:text-zinc-400">Enter the server configuration details in JSON format. Ensure the structure is correct before saving.</p>
            </div>
          </section>
        </div>
      </main>
