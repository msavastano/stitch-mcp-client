...
const ManageServerToolsPage = ({
  ...
  useEffect(() => {
    getServer(serverId).then((data: Server | undefined) => {
      if (data) {
        setServer(data);
      }
      setLoading(false);
      ...
      <header className="sticky top-0 z-10 w-full bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-sm">
        <div className="flex items-center p-4 pb-2 justify-between border-b border-gray-200/10 dark:border-white/10">
          <div className="flex items-center text-gray-500 dark:text-gray-400">
            ...
            <h2 className="text-gray-600 dark:text-gray-300 text-sm font-medium leading-tight tracking-[-0.015em] ml-2">
              Back to Servers
            </h2>
          </div>
        </div>
      ...
      <main className="flex flex-1 flex-col lg:flex-row">
        <aside className="w-full lg:w-80 xl:w-96 flex-shrink-0 border-b lg:border-b-0 lg:border-r border-gray-200/10 dark:border-white/10 p-4 md:p-6">
          <div className="flex flex-col items-stretch justify-start rounded-lg bg-gray-500/5 p-4">
            ...
            <p className="text-gray-900 dark:text-white text-lg font-bold leading-tight tracking-[-0.015em] mt-2">
              {server.name}
            </p>
        ...
        <div className="flex-1 p-4 md:p-6">
          <div className="max-w-7xl mx-auto">
            <h1 className="text-gray-900 dark:text-white tracking-light text-[32px] font-bold leading-tight">
              Manage Tools for {server.name}
            </h1>
                  ...
                  <thead className="bg-gray-500/5">
                    <tr>
                      <th className="p-4 w-12">
                        <input
                          className="form-checkbox rounded bg-transparent border-gray-500/50 text-primary focus:ring-primary/50"
                          type="checkbox"
                        />
                      </th>
                      ...
                      <tr key={tool.id} className="hover:bg-gray-500/5">
                        <td className="p-4 w-12">
                          <input
                            className="form-checkbox rounded bg-transparent border-gray-500/50 text-primary focus:ring-primary/50"
                            type="checkbox"
                            defaultChecked={tool.status === "Selected"}
                          />
                        </td>
                        ...
                        <td className="p-4 text-gray-600 dark:text-gray-400 align-top max-w-sm">
                          {tool.description}
                        </td>
                        <td className="p-4 align-top">
                          <div
                            className={`inline-flex items-center gap-2 rounded-full py-1 px-3 ${
                              tool.status === "Selected"
